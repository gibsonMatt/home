...
function parseBibs() {
    ...
    const fileContents: string = fs.readFileSync('app/publications/pubs.bibtex').toString()
    ...
    bibs.forEach((entry: object) => {
        ...
        const fields: RawField[] = Object.entries(entry['fields']).map(([key, val]) => {
                        ...
                        <div>
                            {!entry.url &&
                                <div className='pb-4 hover:text-blue-600'>
                                    {entry.author}. <span className='font-bold'>{entry.year}</span>. {entry.title}. <span className='italic'>{entry.journal}</span>
                                </div>
                            }
                            {entry.url &&
                                <div className='pb-4 hover:text-blue-600'>
